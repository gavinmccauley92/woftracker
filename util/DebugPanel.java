...
public class DebugPanel extends JPanel {
	...
	public DebugPanel() {
		...
		jSP.setRowHeaderView(new TextLineNumber(log));

		MessageConsole mc = new MessageConsole(log);
		mc.redirectOut(Color.BLUE, null);
		mc.redirectErr(Color.RED, null);
		mc.setMessageLines(LINE_LIMIT);
		
		add(new JLabel("Debug info is in blue, Java uncaught errors are in red. The log maxes at " + LINE_LIMIT + " lines.", SwingConstants.CENTER), BorderLayout.NORTH);
	...
	class ConsoleOutputStream extends ByteArrayOutputStream
	{
		...
		public ConsoleOutputStream(Color textColor, PrintStream printStream)
		{
			if (textColor != null)
			{
				...
				//StyleConstants.setAlignment(attributes, StyleConstants.ALIGN_CENTER);	//my addition
			}
		...
		private void clearBuffer()
		{
			...
			try
			{
				...
				if(textComponent instanceof JTextPane) {
					...
					doc.setParagraphAttributes(0, doc.getLength(), center, false);	//merge w/ color
				}
			}
 ...
 *  With some Java 8 enhancements from me
 *
...
class LimitLinesDocumentListener implements DocumentListener
{
	...
	private BiConsumer<Document, Element> remove;
	...
	public LimitLinesDocumentListener(int maximumLines, boolean isRemoveFromStart)
	{
		...
		remove = isRemoveFromStart ? this::removeFromStart : this::removeFromEnd;
	}
